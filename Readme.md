...
The current workflow is notebook-first, with script entry points for diffusion training. The main reusable code lives in `pipeline/` and `sh_models/`, with Mitsuba-based scene generation under `pipeline/`.
...
├── resize_loader_demo.ipynb      # residual-loader resize and mask visualisation
├── runtraining.py                # diffusion/residual training entry script
├── specdiff_train.py             # diffusion trainer and checkpoint/error logging
├── testnotebook.ipynb
├── logs/
│   └── error_log.txt             # runtime training errors
├── pipeline/
│   ├── Diffuser.py               # diffusion schedule / forward process helpers
...
├── sh_models/
│   ├── DiT.py                    # transformer diffusion backbone components
│   ├── d_backbone.py             # residual U-Net and transformer-backed diffusion models
...
│   ├── train/
│   └── test/
├── outputs/                      # qualitative predictions and figures
└── papers/
    └── spec.tex
```
...
- `resize_loader_demo.ipynb`
  Demonstrates `pipeline/ResidualLoader.py` with `resize_hw=None`, square resize, and non-square resize, and visualises the returned RGB inputs, residuals, soft masks, and binary masks.

- `runtraining.py`
  Script entry point for the current diffusion-style residual training runs.
...
- The current workflow is notebook-first, but the diffusion/residual training path now also has script entry points in `runtraining.py` and `specdiff_train.py`.
- `pipeline/PSDloader.py` loads paired glossy/diffuse images directly, and `pipeline/ResidualLoader.py` builds residual targets plus highlight masks.
- `sh_models/d_backbone.py` contains the current residual U-Net and transformer-backed diffusion backbones, with transformer blocks defined in `sh_models/DiT.py`.
