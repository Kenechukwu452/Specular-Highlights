...
import math
from . import DiT as Transformer
...
class EmbedTime(nn.Module):
    ...
    def forward(self, x, t):
        ...
        emb = emb.reshape(emb.shape[0], emb.shape[1], 1, 1).expand(-1, -1, x.shape[-2], x.shape[-1])
        return x + emb
...
class LatentUNetWithAttention(nn.Module):
    def __init__(self,noise_steps: int = 1000, time_dim: int = 256, tran: bool = False,depth: int = 2):
        """ Initialize UNet with Attention """  
        super().__init__()
        self.time_dim = time_dim
        ...
        """ Final output layer """
        self.final_conv = nn.Conv2d(self.base_channels, 4, kernel_size=1)
...
def DiT(**kwargs):
    return Transformer.Transformer_S_2(learn_sigma = False,**kwargs)
