...
class _TransformerBlk_SelfCross(nn.Module):
    """
    Pre-norm self-attn + cross-attn (to depth tokens) + MLP.
    Reuses your existing _PreNorm, _SelfAttn, _MLP.
    """
    def __init__(self, dim, heads=8, drop=0.0, mlp_ratio=4.0):
        super().__init__()
        self.self_attn = _PreNorm(dim, _SelfAttn(dim, heads, drop))
        self.cross_attn = _PreNorm(dim, _CrossAttn(dim, heads, drop))
        self.mlp = _PreNorm(dim, _MLP(dim, mlp_ratio, drop))

    def forward(self, x, depth_tokens):
        x = x + self.self_attn(x)
        x = x + self.cross_attn(x, depth_tokens)
        x = x + self.mlp(x)
        return x
...
class TokenInpainter_DepthCross(nn.Module):
    """
    Like your TokenInpainter_Prior, but uses an extra depth image as KV context via cross-attention.
    Inputs:
        T:     [B, N, C]
        pm:    [B, N] boolean (True = hole)
        depth: [B,1,Hd,Wd] or [B,Hd,Wd] or [1,Hd,Wd]
    Output:
        [B, N, C]
    """
    def __init__(
        ...
        use_final_norm=True,
        seed_noise_std=0.01,
        depth_embed_mlp_ratio=2.0,
        depth_norm=True,
        mask_cross_only=False,   # if True, only masked queries get cross-attn update
    ):
        super().__init__()
        self.blocks = nn.ModuleList(
            [_TransformerBlk_SelfCross(dim, heads, drop, mlp_ratio=4.0) for _ in range(depth)]
        )
        self.out_proj = nn.Linear(dim, dim, bias=True)
        ...
        self._final_norm = nn.LayerNorm(dim) if use_final_norm else None
        self._seed_noise_std = seed_noise_std
        self.mask_cross_only = mask_cross_only

        hid = int(dim * depth_embed_mlp_ratio)
        ...
        self.depth_norm = nn.LayerNorm(dim) if depth_norm else None

    def forward(self, T: torch.Tensor, patch_inpaint_mask: torch.Tensor, depth: torch.Tensor):
        B, N, C = T.shape
        device = T.device
        ...
        # seed masked
        X = torch.where(mask, self.mask_token.expand(B, N, C), T)

        if self.training and self._seed_noise_std and self._seed_noise_std > 0:
            noise = torch.randn_like(X) * float(self._seed_noise_std)
            X = torch.where(mask, X + noise, X)

        # pos enc (reuse your existing _build_2d_sincos_pos_embed defined in your script)
        if self.use_positional_encoding:
            pos = _build_2d_sincos_pos_embed(H, W, C, device).expand(B, N, C)
            X = X + pos

        # mask indicator
        X = X + torch.where(mask, self.mask_indicator.expand(B, N, C), torch.zeros_like(X))

        # depth -> tokens (KV)
        depth_tokens = self._depth_to_tokens(depth, H, W, B, device, T.dtype)  # [B,N,C]

        # blocks
        for blk in self.blocks:
            if self.mask_cross_only:
                X_upd = blk(X, depth_tokens)
                X = torch.where(mask, X_upd, X)
            else:
                X = blk(X, depth_tokens)
    ...
    def _depth_to_tokens(self, depth, H, W, B, device, dtype):
        if depth.dim() == 3:
            depth = depth.unsqueeze(1)  # [B,1,Hd,Wd] or [1,1,Hd,Wd]
        elif depth.dim() != 4:
            raise ValueError("depth must be [B,1,H,W], [B,H,W], or [1,H,W]")

        if depth.size(0) == 1 and B > 1:
            depth = depth.expand(B, -1, -1, -1)
        ...
        d = depth.reshape(B, 1, H * W).transpose(1, 2)  # [B,N,1]
        dtok = self.depth_embed(d)  # [B,N,C]
        if self.depth_norm is not None:
            dtok = self.depth_norm(dtok)
        return dtok